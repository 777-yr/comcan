...
class CANControlGUI:
    ...
    def open_configure_window(self):
        ...
        ttk.Button(config_window, text="Save Configuration", command=self.save_configuration).grid(row=9, column=0, padx=5, pady=5)
        ttk.Button(config_window, text="Load Configuration", command=self.load_configuration).grid(row=9, column=1, padx=5, pady=5)
        ttk.Button(config_window, text="Close", command=lambda: self.close_config_window(config_window)).grid(row=10, column=0, columnspan=3, pady=10)
