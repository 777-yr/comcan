...
class CANControlGUI:
    ...
    def handle_wakeup_signal(self, *args):
        if self.wakeup_signal_active.get():
            self.start_sending_wakeup_message()
        else:
            self.stop_sending_wakeup_message()

    def start_sending_wakeup_message(self):
        ...
        self.wakeup_timer = self.root.after(10, self.start_sending_wakeup_message)

    def stop_sending_wakeup_message(self):
        if hasattr(self, 'wakeup_timer'):
            self.root.after_cancel(self.wakeup_timer)
